...
const { saveDocument, readDocument } = require('./documentsStorage');
...
// Streams a submitted document (CNIC, utility bill, etc.) to the staff
// dashboard. Documents are stored privately (see documentsStorage.js) —
// this route is the only way to read them back, and it sits behind the
// same testing-phase access gate as the rest of the site.
app.get('/api/staff/documents/:applicationId/:field', async (req, res) => {
  try {
    const { applicationId, field } = req.params;
    const applications = await readApplications();
    const application = applications.find((a) => a.applicationId === applicationId);
    const ref = application && application.documents && application.documents[field];
    if (!ref) {
    ...
    console.error('Document read error:', err && err.message ? err.message : err);
    return res.status(500).send('Could not load document.');
  }
});
